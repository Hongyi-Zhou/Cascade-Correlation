...
class RNN_Phoneme_Classifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l in range(0, self.num_layers):
                h = self.rnn[l].forward(h_last, self.hiddens[-1][l])
                hidden[l] = h
                h_last = h
    ...
    def backward(self, delta):
        ...
        #dh[-1] = self.output_layer.backward(delta.reshape((-1, delta.shape[-1])))
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1, -1, -1):
            for l in range(self.num_layers-1, -1, -1):
                ...
                if l != 0:
                    h_prev_l = self.hiddens[-1][l-1]
                else:
                    h_prev_l = self.x[:,t,:]

                h_prev_t = self.hiddens[-2][l]
                h = self.hiddens[-1][l]

                dx_, dh_ = self.rnn[l].backward(dh[l-self.num_layers], h, h_prev_l, h_prev_t)
                dh[l-self.num_layers] = dh_

                if l != 0:
                    dh[l-self.num_layers-1] += dx_
            self.hiddens.pop(-1)
